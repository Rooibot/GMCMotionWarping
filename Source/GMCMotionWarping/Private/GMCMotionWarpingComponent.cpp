...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GMCMotionWarpingComponent)
...
FTransform UMotionWarpingUtilities::CalculateRootTransformRelativeToWarpPointAtTime(APawn* Character, const UAnimSequenceBase* Animation, float Time, const FName& WarpPointBoneName)
{
	IGMCMotionWarpInterface* MotionWarpInterface =Cast<IGMCMotionWarpInterface>(Character);
	if (const USkeletalMeshComponent* Mesh = MotionWarpInterface->GetMotionWarpMesh())
	{
		if (const UAnimInstance* AnimInstance = Mesh->GetAnimInstance())
		{
			...
			if (BoneIndex != INDEX_NONE)
			{
				...
				const FTransform MeshCompRelativeRotInverse = FTransform(MotionWarpInterface->GetRotationOffset().Inverse());
...
FTransform UMotionWarpingUtilities::CalculateRootTransformRelativeToWarpPointAtTime(APawn* Character, const UAnimSequenceBase* Animation, float Time, const FTransform& WarpPointTransform)
{
	IGMCMotionWarpInterface* MotionWarpInterface =Cast<IGMCMotionWarpInterface>(Character);
	// Inverse of mesh's relative rotation. Used to convert root and warp point in the animation from Y forward to X forward
	const FTransform MeshCompRelativeRotInverse = FTransform(MotionWarpInterface->GetRotationOffset().Inverse());
...
{
	APawn* Character = GetCharacterOwner();
	check(Character);

	FMotionWarpingUpdateContext Context;
	Context.DeltaSeconds = DeltaSeconds;
	UGMC_OrganicMovementCmp* GMC = MotionWarpInterfacePawn->GetOrganicMovementCmp();
	if(!GMC) return;
...
FTransform UMotionWarpingComponent::ProcessRootMotionPreConvertToWorld(const FTransform& InRootMotion, UGMC_OrganicMovementCmp* GeneralMovementComponent, float DeltaSeconds)
{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
