...
#include "GMCMotionWarpingComponent.generated.h"
...
struct GMCMOTIONWARPING_API FMotionWarpingCVars
{
...
UCLASS()
class GMCMOTIONWARPING_API UMotionWarpingUtilities : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	static FTransform CalculateRootTransformRelativeToWarpPointAtTime(APawn* Character, const UAnimSequenceBase* Animation, float Time, const FName& WarpPointBoneName);

	/** @return root transform relative to the warp point transform at the supplied time */
	static FTransform CalculateRootTransformRelativeToWarpPointAtTime(APawn* Character, const UAnimSequenceBase* Animation, float Time, const FTransform& WarpPointTransform);
};
...
class GMCMOTIONWARPING_API UMotionWarpingComponent : public UActorComponent
{
	GENERATED_BODY()
...
public:
	...
	FORCEINLINE APawn* GetCharacterOwner() const { return CharacterOwner.Get(); }
...
protected:
	...
	UPROPERTY(Transient)
	TWeakObjectPtr<APawn> CharacterOwner;

	class IGMCMotionWarpInterface* MotionWarpInterfacePawn;
	...
	FTransform ProcessRootMotionPreConvertToWorld(const FTransform& InRootMotion, UGMC_OrganicMovementCmp* CharacterMovementComponent, float DeltaSeconds);
};
