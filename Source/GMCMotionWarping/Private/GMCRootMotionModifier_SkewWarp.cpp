...
{
	APawn* CharacterOwner = GetCharacterOwner();
	IGMCMotionWarpInterface* MotionWarpInterfacePawn = Cast<IGMCMotionWarpInterface>(CharacterOwner);
	...
	if (bWarpTranslation)
	{
		const float CapsuleHalfHeight = MotionWarpInterfacePawn->GetCollisionHalfHeight();
		...
		if (!TotalTranslation.IsNearlyZero())
		{
			if (!DeltaTranslation.IsNearlyZero())
			{
				const FTransform MeshTransform = FTransform(MotionWarpInterfacePawn->GetRotationOffset(), MotionWarpInterfacePawn->GetTranslationOffset()) * CharacterOwner->GetActorTransform();
